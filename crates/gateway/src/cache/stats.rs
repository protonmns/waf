...
impl TimeseriesBucket {
    fn new(ts: u64, hits: u64, misses: u64, stores: u64, memory_used_bytes: u64) -> Self {
        ...
        Self { ts, hits, misses, hit_ratio, stores, memory_used_bytes }
    }
}
...
impl Default for TimeseriesRing {
    fn default() -> Self {
        Self { buckets: VecDeque::new(), max_buckets: 60 }
    }
}
...
impl CacheStats {
    ...
    pub fn tick_timeseries(&self, memory_used_bytes: u64) {
        ...
        *baseline = BucketBaseline { hits: hits_now, misses: misses_now, stores: stores_now };
        drop(baseline);
